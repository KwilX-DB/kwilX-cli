...
class kwilxTerminal {
  constructor(kwilxCLI, callback = (type, msg) => {}) {
    this.app = kwilxCLI;
    this.cb = callback;

    // options
    this.autoSync = kwilxCLI.options.autoSync;
    this.debug = kwilxCLI.options.debug;
    this.sync = kwilxCLI.options.sync;
    this.record = kwilxCLI.options.outputFile ? true : false;
  ...
  printHelp() {
    ...
    console.log("  sync     (\\S) enable kwilx query sync");
    console.log("  nosync   (\\s) disable kwilx query sync");
    ...
    try {
      const answers = await inquirer.prompt([
        {
          type: "command",
          name: "cmd",
          message: `kwilx [${this.app.getConnectedMoatName() || "(none)"}]>`,
          autoCompletion: [
            "?",
            "\\?",
            "\\D",
      ...
      switch (answers.cmd) {
        ...
        case "\\i":
          ...
          else console.log("ERROR: kwilx moat not connected");
          break;
        ...
        default:
          ...
          // require connection to kwilx moat
          if (!this.app.getConnectedMoatName()) {
            console.log("ERROR: kwilx moat not connected. Use 'connect'.");
            break;
          }
