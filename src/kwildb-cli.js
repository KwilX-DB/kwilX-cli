const kwilx = require("kwilx");
const fs = require("fs");
const inquirer = require("inquirer");
const nReadlines = require("n-readlines");

const ProgressBar = require("./progress-bar.js");
const kwilxTerminal = require("./kwilx-terminal.js");
...
class kwilxCLI {
  constructor(options, pkg) {
    ...
    this.info = {
      engine: undefined, // kwilx database engine
      // moat funds usage
      funds: {
        total: undefined,
        value: undefined,
  ...
  async init() {
    try {
      ...
      if (this.options.inputFile) {
        ...
        if (this.options.moat === undefined) {
          console.log("ERROR: kwilx moat must be specified");
          this.exit(1);
        }
  ...
  async terminalPrompt() {
    ...
    const terminal = new kwilxTerminal(this, (type, msg) => {
      switch (type) {
        case "ERROR":
  ...
  async query(q, sync, options = this.options) {
    ...
    // NOTE: hack!!! reassign console.log to prevent kwilx from making console noise for each query
    // TOOD: revisit with the evolution of kwilx library
    const log = console.log;
    if (!options.debug) console.log = () => {};

    try {
      let result = await this.kwilx.query(q, sync);
      console.log = log;
  ...
  async selectMoatConfig() {
    try {
      ...
      const answers = await inquirer.prompt([
        {
          type: "search-list",
          message: "Select kwilx moat configuration or create a new one",
          name: "moatConfig",
          choices,
        },
      ]);
  ...
  readMoatConfigFile(moat) {
    ...
    config.kwilx.privateKey = JSON.parse(config.kwilx.privateKey);
    return config;
  }
  ...
  //   try {
  //     const socket = this.kwilx.createWebSocket();
  ...
  async connectToMoat(config, exitOnError = true) {
    this.kwilx = kwilx.createConnector(config.kwilx, config.kwilx.secret);
    const info = await this.getMoatInfo(this.kwilx);

    if (info === null) {
      console.log("ERROR: failed to access kwilx moat");
      if (exitOnError) this.exit(1);
      this.options.moat = undefined;
      return null;
    } else {
      this.options.moat = config.kwilx.moat;
  ...
  async getMoatInfo(kwilx) {
    try {
      const funding = await kwilx.getMoatFunding();
      const debit = await kwilx.getMoatDebit();
  ...
  async refreshConnectedMoatInfo() {
    ...
    const moatInfo = await this.getMoatInfo(this.kwilx);
  ...
  printMoatInfo() {
    ...
    console.log(`moat:     ${config.kwilx.moat}`);
    console.log(`host:     ${config.kwilx.host}`);
    if (config.kwilx.port) console.log(`port:     ${config.kwilx.port}`);
    console.log(`protocol: ${config.kwilx.protocol}`);
